...
assert.equal(validateBrowserUrl("https://EXAMPLE.test/path", allowed).hostname, "example.test");
...
for (const raw of [
	...
	throwsCode(() => validateBrowserUrl(raw), raw.includes("not-allowed") ? "disallowed-host" : "invalid-url", `rejects ${raw.split(":")[0]} or unsafe host`);
}
throwsCode(() => parseAllowedHosts("*.example.test"), "disallowed-host", "wildcard allowlist entries are rejected");
throwsCode(() => parseAllowedHosts("example.test:8443"), "disallowed-host", "port allowlist entries are rejected");

assert.equal(validateCdpEndpoint("http://127.0.0.1:9222/"), "http://127.0.0.1:9222/");
throwsCode(() => validateCdpEndpoint("http://192.0.2.1:9222"), "invalid-cdp-endpoint", "non-loopback CDP endpoint is rejected");
throwsCode(() => validateCdpEndpoint("ws://127.0.0.1:9222"), "invalid-cdp-endpoint", "WebSocket CDP endpoint is rejected");
throwsCode(() => validateCdpEndpoint("http://user:pass@127.0.0.1:9222"), "invalid-cdp-endpoint", "CDP credentials are rejected");
...
{
	const args = buildLaunchArgs({ profileDir: path.join("tmp", "isolated"), port: 45678, headless: true });
...
{
	assert.equal(resolveBrowserBinary({ binary: process.execPath }), process.execPath);
	throwsCode(
		() => resolveBrowserBinary({ binary: "missing-browser" }, { statSync: () => { throw new Error("missing"); } }),
		"browser-unavailable",
...
{
	const browserSource = fs.readFileSync(path.join(__dirname, "../extensions/pi_minimal_webui/browser.ts"), "utf8");
	const indexSource = fs.readFileSync(path.join(__dirname, "../extensions/pi_minimal_webui/index.ts"), "utf8");
	const names = [...browserSource.matchAll(/name: "(browser_[^"]+)"/g)].map((match) => match[1]);
	assert.deepEqual(names, ["browser_open", "browser_snapshot", "browser_screenshot", "browser_console"]);
	...
	const openVerdict = policy.resolve("browser_open", "http://127.0.0.1:4317", layers);
	...
	assert.equal(policy.applyMode(snapshotVerdict, "read-only", { toolName: "browser_snapshot" }).action, "allow");
	assert.equal(policy.applyMode(openVerdict, "read-only", { toolName: "browser_open" }).action, "deny");
	ok("extension registration and safeguard defaults cover only the four MVP tools");
}

{
	...
	const raw = {
		...
		elements: [
			{ index: 0, role: "button", name: "Send", text: "Send", rect: { x: 1, y: 2, width: 72, height: 32 }, href: "https://example.test/send" },
			{ index: 1, type: "password", value: "secret", rect: { x: 0, y: 0, width: 10, height: 10 } },
			{ index: 2, role: "link", href: ["java", "script"].join("") + ":alert(1)", rect: { x: 0, y: 0, width: 10, height: 10 } },
			{ index: 3, role: "broken", rect: { x: NaN, y: 0, width: 1, height: 1 } },
			...Array.from({ length: MAX_ELEMENTS + 5 }, (_, index) => ({ index: index + 4, role: "button", name: `B${index}`, rect: { x: 0, y: 0, width: 1, height: 1 } })),
		],
	};
	...
	assert.ok(Buffer.byteLength(snapshot.pageText, "utf8") <= MAX_PAGE_TEXT_BYTES);
	...
	const changed = normalizeSnapshot({ ...raw, elements: [{ ...raw.elements[0], name: "Changed" }] }, store);
	assert.notEqual(changed.elements[0].ref, "e1");
	throwsCode(() => store.resolve("e1"), "stale-ref", "changed snapshot invalidates old refs");
	store.invalidate();
	throwsCode(() => store.resolve("e1"), "stale-ref", "navigation invalidates refs");
	const empty = normalizeSnapshot({ url: "http://localhost:4317", viewport: { width: 1, height: 1 } }, store);
...

{
	...
	assert.equal(collector.push("Runtime.consoleAPICalled", { type: "log", args: [{ type: "string", value: "ignored" }] }), false);
	assert.equal(collector.push("Runtime.consoleAPICalled", {
		type: "warning",
		args: [{ type: "string", value: "warning" }, { type: "number", value: 7 }],
		url: "https://example.test/app.js",
		lineNumber: 4,
		columnNumber: 2,
	}), true);
	assert.equal(collector.push("Runtime.exceptionThrown", {
		exceptionDetails: { text: "boom", url: "https://example.test/app.js", lineNumber: 9, columnNumber: 1 },
	}), true);
	for (let i = 0; i < MAX_ENTRIES + 10; i++) collector.push("Runtime.consoleAPICalled", { type: "error", text: `error-${i}` });
	...
	assert.ok(Buffer.byteLength(result.entries.at(-1).text, "utf8") <= MAX_TEXT_BYTES);
	assert.equal(result.entries.some((entry) => entry.text === "ignored"), false);
	ok("console warnings/errors are normalized and capped at 50 entries");

	const subscriptions = new Map();
	const session = { on(method, handler) { subscriptions.set(method, handler); return () => subscriptions.delete(method); } };
	const subscribed = new ConsoleCollector();
	const cleanup = subscribed.subscribe(session);
	subscriptions.get("Runtime.consoleAPICalled")({ type: "error", text: "from event" });
...
(async () => {
	...
	const resized = await captureScreenshot({
		command: async (_method, params) => {
			...
			return { data: Buffer.alloc(params.quality > 40 ? MAX_SCREENSHOT_BYTES + 1 : 8).toString("base64") };
		},
	}, { imageSupported: true, url: "http://localhost:4317", viewport: { width: 99999, height: 99999 } });
	...
	const textOnly = await captureScreenshot({ command: async () => { throw new Error("must not capture"); } }, {
	...
	await assert.rejects(
		captureScreenshot({ command: async () => ({ data: Buffer.alloc(MAX_SCREENSHOT_BYTES + 1).toString("base64") }) }, {
			imageSupported: true,
			url: "http://localhost:4317",
		}),
	...
	await assert.rejects(
		captureScreenshot({ command: async () => { throw abortError; } }, { imageSupported: true, signal: {} }),
	...
	const handle = await launchManagedBrowser(
		{ binary: process.execPath, headless: false },
		{
			...
			spawn: (binary, args, options) => {
				const child = { pid: 99, kill: (signal) => killed.push([child.pid, signal]) };
