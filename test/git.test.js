/*
 * git.test.js — node:assert/strict tests for git.js pure parsers.
 * (getGitSnapshot/getGitFileDiff need a real git repo; covered by the live
 * smoke test below, not here.) Plan 4.5.
 */
"use strict";
...
(function () {
	...
	ok("file.js modified", changes[0].path === "file.js" && changes[0].status === "modified");
	ok("new.txt added (untracked)", changes[1].path === "new.txt" && changes[1].status === "added");
	ok("staged.js added", changes[2].path === "staged.js" && changes[2].status === "added");
})();
...
(function () {
	...
	ok("rename target is new.js", changes[0].path === "new.js" && changes[0].status === "renamed");
	ok("next entry parsed correctly after rename", changes[1].path === "other.js");
})();
...
(function () {
	...
	ok("a.js +10 -2", counts[0].path === "src/a.js" && counts[0].additions === 10 && counts[0].deletions === 2);
	ok("binary deletions null", counts[1].path === "binary.png" && counts[1].deletions === null);
})();
...
(function () {
	...
	ok("new path entry", counts[0].path === "new.js" && counts[0].additions === 5);
	ok("old path entry", counts[1].path === "old.js" && counts[1].deletions === 3);
})();
...
(function () {
	...
	ok("a.js merged +15 -3", merged.get("a.js").additions === 15 && merged.get("a.js").deletions === 3);
	ok("b.js +3 -0", merged.get("b.js").additions === 3 && merged.get("b.js").deletions === 0);
})();
...
(function () {
	...
	ok("binary null + number → number (not NaN)", merged.get("bin.png").additions === 4);
})();
...
(function () {
	...
	ok("a.js modified", changes[0].path === "src/a.js" && changes[0].status === "modified");
	ok("b.js added", changes[1].path === "src/b.js" && changes[1].status === "added");
	ok("rename → new.js renamed", changes[2].path === "new.js" && changes[2].status === "renamed");
})();
